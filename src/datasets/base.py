...
class AbstractDataset(metaclass=ABCMeta):
    def __init__(self, target_behavior, multi_behavior, min_uc, predict_only_target=False, split='leave_one_out', develop=False):
        ...
        # self.bmaps = {1: 'neutral', 2: 'tip', 3: 'pos', 4: 'neg'}  # yelp
        self.bmaps = {1: 'fav', 2: 'cart', 3: 'buy', 4: 'pv'}  # retail, ijcai
        if self.develop:
            self.min_uc += 1
    ...
    def split_df(self, df, user_count):
        ...
        for user in range(1, user_count+1):
            ...
            # develop stage, throw the test item away.
            if self.develop:
                items, behaviors = items[:-1], behaviors[:-1]
